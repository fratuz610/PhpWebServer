...
public class FastCGIHandler {
  ...
  public FastCGIHandler(FastCGIErrorHandler errorHandler) {
    FastCGIHandler.errorHandler = errorHandler;
  }
  ...
  private static FastCGIErrorHandler errorHandler;
  
	private static final long serialVersionUID = 7733797250176758803L;

	private static final Logger log = Logger.getLogger(FastCGIHandler.class.getSimpleName());
	...
	private int parseHeaders(ResponseAdapter res, InputStream is)
		...
		if (ch < 0) {
			log.severe("parseHeaders: Can't contact FastCGI");
	...
	static class FastCGIInputStream extends InputStream {
		...
		private boolean readNext() throws IOException {
          ...
          if(errorHandler != null)
            errorHandler.logError(new String(buf, 0, length));
				...
				default:
					log.info(_fcgiSocket + ": Unknown Protocol(" + type + ")");
